...
int main(int argc, char **argv){
    ...
    const bool CAPTURE_ALL_SHAPES = true;
    bool CAPTURE_ALL_SHAPE_DATA = true;
    ...
    for(int shape=0; shape<SHAPE_NAMES.size(); shape++){
        for(int angle=0; angle<SHAPE_CAPTURE_ANGLES[shape].size(); angle++){
            ...
            createDirectory(MESH_DIR+SHAPE_NAMES[shape]+SHAPE_CAPTURE_ANGLES[shape][angle]);
            string out_file="";
            if(USING_FEATURES) out_file+="feature_mesh.ply";
            else out_file+= "smooth_mesh.ply";
            string out_path (MESH_DIR+SHAPE_NAMES[shape]+SHAPE_CAPTURE_ANGLES[shape][angle]+out_file);
