function [faces,vertices] = ExtractSeparatingSurfaceGPU(V,niter)

%ExtractSeparatingSurface
...
%V. Lempitsky, "Surface Extraction from Binary Volumes with Higher-Order Smoothness", MSR-TR-2009-31 (also in submission).
   ...
   % close all;
    ...
    %MATLAB will always include the outer boundary in the perimeter, so undo
    %this:
    perim(:,:,1) = 0;
    perim(:,:,end) = 0;
    perim(:,1,:) = 0;
    ...
    lb = single(lbnd(where));
    ub = single(ubnd(where));
    ...
    x = QPGPU(R, invdg, lb, ub, x, double(niter), 12.0);
    toc
